...
- 文本聊天：发送消息、流式显示、停止生成、编辑重发、继续生成、重新生成、复制、分享、删除。
...
- 模型测试：在模型编辑页可测试 Base URL、API Key 和模型名称是否可用。
...
- 修复 0.4.0 中键盘弹出后输入框被遮挡的问题。
- 用户消息支持“编辑并重发”，会删除后续 AI 回复并按新问题重新生成。
- AI 回复支持“继续生成”，用于回答中断或内容不完整时续写。
- 单条消息支持调用系统分享面板，方便发到微信、备忘录等 App。
- 模型编辑页新增“测试模型连接”，用于检查 Base URL、API Key 和模型名称。
...
从 0.4.0 升级到 0.5.0 时，可以直接按 [docs/UPGRADE_0_5_0.md](docs/UPGRADE_0_5_0.md) 操作。
